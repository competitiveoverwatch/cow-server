...
def get_flairdata(temp=False):
	flairdata = None
	if temp:
		with open("static/data/flairs_tmp.json") as flair_data:
			flairdata = json.load(flair_data)
	else:
		with open("static/data/flairs.json") as flair_data:
			flairdata = json.load(flair_data)
	return flairdata
...
def set_flairdata(flairdata, temp=False):
	if temp:
		with open('static/data/flairs_tmp.json', 'w') as flair_data:
